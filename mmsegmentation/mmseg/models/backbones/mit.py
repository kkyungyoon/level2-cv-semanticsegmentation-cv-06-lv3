...
class CustomMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        for i, num_layer in enumerate(num_layers):
            ...
            # 첫 번째 PatchEmbed의 projection 수정---------------------
            if i == 0:
    ...
    def forward(self, x):
        ...
        # Down Conv layer 추가---------------------
        x = self.downconv(x)
        # print(f"After DownConv: {x.shape}")
        # ----------------------------------------
        for i, layer in enumerate(self.layers):
            # print(layer[0])
            x, hw_shape = layer[0](x)
            # print(f"Stage {i} PatchEmbed output: {x.shape}")
            for block in layer[1]:
                x = block(x, hw_shape)
                # print(f"Stage {i} Transformer block output: {x.shape}")
            x = layer[2](x)
            # print(f"Stage {i} normalized output: {x.shape}")
            x = nlc_to_nchw(x, hw_shape)
            if i in self.out_indices:
                outs.append(x)
