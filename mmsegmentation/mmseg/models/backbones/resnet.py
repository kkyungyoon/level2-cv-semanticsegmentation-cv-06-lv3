...
class CustomResNet(BaseModule):
    ...
    def __init__(self,
        ...
        # Down Conv layer 추가------------------------------------------------------------------------
        # self.downconv = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=2, padding=1)
        # -------------------------------------------------------------------------------------------
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        ...
        if self.deep_stem:
            self.stem = nn.Sequential(
                # Down Conv layer 추가로 인한 조정-------------------------------------------------
                # nn.Conv2d(3, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False),
                # build_norm_layer(self.norm_cfg, 32)[1],  # 출력 채널 32와 일치
                # nn.ReLU(inplace=True),
                # -----------------------------------------------------------------------------
                build_conv_layer(
                    self.conv_cfg,
...
@MODELS.register_module()
class CustomResNetV1c(CustomResNet):
    """ResNetV1c variant described in [1]_.
...
@MODELS.register_module()
class CustomResNet2(ResNet):
    """Custom ResNet backbone."""

    def __init__(self, **kwargs):
        norm_cfg = kwargs.pop('norm_cfg', dict(type='GN', num_groups=32))
        super().__init__(norm_cfg=norm_cfg, **kwargs)